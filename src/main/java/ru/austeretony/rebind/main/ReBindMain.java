...
public class ReBindMain {
		...
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (fields[i].getName().equals("KEYBIND_ARRAY") || fields[i].getName().equals("field_74516_a")) {
					...
					List<KeyBinding> list = (List<KeyBinding>) fields[i].get(null);
	...
	private void modifyKeyBindings() {
		...
		gameSettings.keyBindSwapHands = new KeyBinding("key.swapHands", this.CONFIG_LOADER.keyCodeSwapHands, this.CATEGORY_INVENTORY);

		if (this.CONFIG_LOADER.enableSwapHands)
		modifiedKeys.add(gameSettings.keyBindSwapHands);
		...
		gameSettings.keyBindFullscreen = new KeyBinding("key.fullscreen", this.CONFIG_LOADER.keyCodeFullscreen, this.CATEGORY_MISC);

		if (this.CONFIG_LOADER.enableFullscreen)
		modifiedKeys.add(gameSettings.keyBindFullscreen);
				
		gameSettings.keyBindSpectatorOutlines = new KeyBinding("key.spectatorOutlines", this.CONFIG_LOADER.keyCodeSpectatorOutlines, this.CATEGORY_MISC);

		if (this.CONFIG_LOADER.enableSpectatorOutlines)
		modifiedKeys.add(gameSettings.keyBindSpectatorOutlines);
    ...
    private void removeUnusedCategories() {
    	...
    	if (!this.CONFIG_LOADER.enableScreenshot && !this.CONFIG_LOADER.enableFullscreen && !this.CONFIG_LOADER.enableTogglePerspective 
    			&& !this.CONFIG_LOADER.enableSpectatorOutlines && !this.CONFIG_LOADER.enableSmoothCamera && !this.CONFIG_LOADER.enableHideGUI 
    			&& !this.CONFIG_LOADER.enableQuit && !this.CONFIG_LOADER.enableDebugMenu && !this.CONFIG_LOADER.enableSwitchShader) {
    		
    		KeyBinding.getKeybinds().remove(this.CATEGORY_MISC);
    	}
    	
    	if (!this.CONFIG_LOADER.enableSwapHands && !this.CONFIG_LOADER.enableDrop && !this.CONFIG_LOADER.enableInventory 
    			&& !this.CONFIG_LOADER.enableHotbar1 && !this.CONFIG_LOADER.enableHotbar2 && !this.CONFIG_LOADER.enableHotbar3 
    			&& !this.CONFIG_LOADER.enableHotbar4 && !this.CONFIG_LOADER.enableHotbar5 && !this.CONFIG_LOADER.enableHotbar6 
    			&& !this.CONFIG_LOADER.enableHotbar7 && !this.CONFIG_LOADER.enableHotbar8 && !this.CONFIG_LOADER.enableHotbar9) {

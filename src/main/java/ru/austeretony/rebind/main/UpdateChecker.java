		...
		if (event.getEntity() instanceof EntityPlayer) {
			...
			if (!notified) { 
		        ...
		        if (this.compareVersions(ReBindMain.VERSION, availableVersion)) {	
		        	...
		        	event.getEntity().addChatMessage(updateMessage1.appendSibling(updateMessage2).appendSibling(updateMessage3));
		        	event.getEntity().addChatMessage(pageMessage1.appendSibling(pageMessage2).appendSibling(urlMessage));
		            ...
		            if (ConfigLoader.shouldShowChangelog()) {
	            		...
	            		event.getEntity().addChatMessage(changelogMessage);
		            		            		
	            		for (String line : changelog)            			            			
	            			event.getEntity().addChatMessage(new TextComponentString(" + " + line));
		            }
		        }
			}
